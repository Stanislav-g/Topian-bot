    ...
    async def module_rep(self, ctx, arg = None):
        ...
        if not arg:
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                on = 'on'
                collectionmodules = db["modules"]
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": on, "lvls": 0, "rep": on, "ticket": 1, "warns": 0, "logs": 0})
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                allnum = str(num) + str(num2)
                off = 'off'
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": off, "lvls": 0, "rep": off, "ticket": 1, "warns": 0, "logs": 0})
