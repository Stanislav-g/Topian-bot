    ...
    async def module_reaction(self, ctx, arg = None):
        ...
        if not arg:
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                allnum = str(num) + str(num2)
                on = 'on'
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": 0, "lvls": on, "rep": 0, "ticket": 1, "warns": 0, "logs": 0, "reaction": on})
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                allnum = str(num) + str(num2)
                off = 'off'
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": 0, "lvls": off, "rep": 0, "ticket": 1, "warns": 0, "logs": 0, "reaction": off})
