    ...
    async def module_reaction(self, ctx, arg = None):
        ...
        if not arg:
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                allnum = str(num) + str(num2)
                on = 'on'
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": 0, "lvls": 0, "rep": 0, "ticket": 1, "warns": 0, "logs": 0, "reaction": on, "roles": 0})
            ...
            if collectionmodules.count_documents({"_id": allnum}) == 0:
                ...
                allnum = str(num) + str(num2)
                off = 'off'
                collectionmodules.insert_one({"_id": allnum, "name": name, "on_off": 0, "lvls": 0, "rep": 0, "ticket": 1, "warns": 0, "logs": 0, "reaction": off, "roles": 0})
    ...
    async def on_message(self, message ):
        ...
        if collectionmodules.count_documents({"_id": allnum4}) == 1:
            if collectionmodules.find_one({"_id": allnum4})["reaction"] == 'on':
                num = str(ctx.author.guild.id) + str(message.channel.id)
                collectionreaction = db["reaction"]
