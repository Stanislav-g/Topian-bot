...
class user(commands.Cog):
    ...
    async def channel_create(self, ctx, *, arg = None): 
        ...
        overwrites.update(guild.overwrites)
	    channel = await guild.create_text_channel(name={arg}, overwrites=overwrites, topic=f'Ticket created by {ctx.author} ({ctx.author.id}) with subject "{subject}"', reason=f'Ticket created by {ctx.author} ({ctx.author.id})')
