...
class user(commands.Cog):
    ...
    async def channel_create(self, ctx, *, arg = None): 
	    ...
	    channel = await guild.create_text_channel(name={arg}, overwrites=overwrites, topic=f'Ticket created by {ctx.author} ({ctx.author.id}) with subject "{arg}"', reason=f'Ticket created by {ctx.author} ({ctx.author.id})')
